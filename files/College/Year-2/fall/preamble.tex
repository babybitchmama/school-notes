...
\pgfplotsset{pccplot/.style={color=linecolor1,mark=none,line width=1pt,solid}}
...
% https://tex.stackexchange.com/a/168978/254196
\newcounter{source}
\newcommand\source[1]{%
...
\newcommand\drawarrows{
  \tikz[remember picture, overlay, bend left=45, -latex] {
    \foreach \j [evaluate=\j as \m using int(\j)] in {1,...,\thesource}{
      \foreach \i [evaluate=\i as \n using int(\i-1)] in {1,...,\thetarget} {
        \draw [linecolor1](source-0.north) to (target-\n.north) coordinate (UP);
      }
    }
  }
  ...
  \tikz[remember picture, overlay, bend left=-45, -latex] {
    \foreach \j [evaluate=\j as \m using int(\j)] in {1,...,\thesource}{
      \foreach \i [evaluate=\i as \n using int(\i-1)] in {1,...,\thetarget} {
        \draw [linecolor2](source-1.south) to (target-\n.south) coordinate (DOWN) ;
      }
    }
  }
}
