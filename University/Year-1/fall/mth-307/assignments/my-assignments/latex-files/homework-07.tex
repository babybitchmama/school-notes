...
\begin{probsolution}
  \begin{enumerate}
    \item Here's the line proof for $D - A \subseteq B - C$.
    ...
    \item Here's the line prove for $A = X \cap A$.
    ...
    \item Here's the line proof for $A = X \cup A$.
...
\begin{probsolution}
  \begin{enumerate}
    \item Here's the line proof for $f(A) - f(B) \subseteq f(A - B)$.
      ...
      \hspace*{-0.5em}\begin{tabular}{ll}
        ...
        5. & Then, there exists $x \in A$ such that $f(x) = y$ and there does not exist $x \in B$ such that $f(x) = y$. \\
        6. & Then, $x \in A$ and $x \notin B$. \\
        7. & Then, $x \in A - B$. \\
        8. & Then, $y \in f(A - B)$. \\
        9. & Therefore, $f(A) - f(B) \subseteq f(A - B)$. \\
      \end{tabular}

    \item Here's the line proof for $f(A - B) \subseteq f(A) - f(B)$.
    ...
    \item Let $S = \{1, 2, 3, 4, 5\}$ and $T = \R$. Let
      \[%
        f(X) = \Card(X) + 1
      .\]%
...
\begin{probsolution}
  \begin{enumerate}
    \item Here's the line proof for $X \cap f^{-1}(W) = \emptyset$.
    ...
    \item Here's the line proof for $A = X \cup f^{-1}(W)$.
    ...
    \item Here's the line proof for $f$ is onto.
      ...
      \hspace*{-0.5em}\begin{tabular}{ll}
        ...
        6. & Case 1: If $b \in f(X)$, then there exists $a \in X$ such that $f(a) = b$ (definition of $f(X)$). \\
        7. & Case 2: If $b \in W$, then there exists $a \in A - X$ such that $f(a) = b$ (since $f(A - X) = W$). \\
...
\begin{probsolution}
  \begin{proof}
    ...
    + 3$ to both sides gives us $1 + 3 + 5 + 7 + \cdots + (2n + 1) + (2n + 3) =
    (n + 1)^2 + (2n + 3)$. Simplifying the right hand side gives us
    \[%
      (n + 1)^2 + (2n + 3) = n^2 + 2n + 1 + 2n + 3 = n^2 + 4n + 4 = (n + 2)^2 = ((n + 1) + 1)
    ,\]%
    which is equivalent to $P(n + 1)$. Hence, $(\forall n \in \N \cup
    \{0\})[P(n) \implies P(n + 1)]$.

    By PMI, we proved that $1 + 3 + 5 + 7 + \cdots + (2n + 1) = (n + 1)^2$ for
    all $n \ge 0$.
  \end{proof}
\end{probsolution}
...
\begin{probsolution}
  \begin{proof}
    ...
    = \left[\frac{n(n + 1)}{2}\right]^2 + (n + 1)^3$. If the statement is true,
...
\begin{probsolution}
  \begin{proof}
    Let $P(n) : \displaystyle\sum_{k=1}^{n} (-1)^k k^2 = (-1)^n \frac{n(n + 1)}{2}$.
...
\begin{probsolution}
  \begin{proof}
    ...
    \]%
    We need to show that $P(n + 1)$, i.e., $\frac{(2(n + 1))!}{(n + 1)! \cdot 2^{n + 1}}$, is also odd. Starting with
    \[
    ...
    .\]%
    By the induction hypothesis, $\frac{(2n)!}{n! \cdot 2^n}$ is odd. Now consider the term
    \[
    ...
    Therefore, $P(n + 1)$ holds, and $\frac{(2n)!}{n! \cdot 2^n}$ is odd for all $n \in \N$ by the principle of mathematical induction.
  \end{proof}
\end{probsolution}
...
\begin{probsolution}
  \begin{proof}
    ...
    Induction Step: Assume $P(n)$ holds for some $n \geq 0$, i.e., assume $a_n \leq 2$. We need to show that $a_{n+1} \leq 2$.
    By the recursive definition of the sequence,
    \[%
      a_{n+1} = \sqrt{2 + a_n}
    .\]%
    Starting with the inductive hypothesis $a_n \leq 2$, we add $2$ to both
    sides
    \[%
      2 + a_n \leq 2 + 2 = 4
    .\]%
...
\begin{probsolution}
  \begin{proof}
    Let $P(n)$ denote the statement $\left(1 + \frac{1}{2}\right)^n > 1 + \frac{n}{2}$.
...
\begin{probsolution}
  \begin{proof}
    ...
    By EPMI, we have proven that $\displaystyle \frac{1}{1^2} + \frac{1}{2^2} + \cdots + \frac{1}{n^2} \leq 2 - \frac{1}{n}$ for all $n \geq 1$.
  \end{proof}
\end{probsolution}
...
\begin{probsolution}
  \begin{enumerate}
    \item $A \subseteq B \iff (\forall x \in A)[x \notin B^c]$.

    \item $A = B \iff A \subseteq B \land B \subseteq A$.

    \item $x \in f(A) \iff (\exists a \in A)[f(a) = x]$.
    \item $y \in f^{-1}(B) \iff f(y) \in B$.
    ...
    \item $f : S \to T$ is onto $\iff (\forall t \in T)(\exists s \in S)[f(s) =
      t]$.

    \item $f : S \to T$ is one-to-one $\iff (\forall s_1, s_2 \in S)[(f(s_1) =
      f(s_2)) \implies (s_1 = s_2)]$.

    \item $x \in A \cup (B - C) \iff x \in A \lor (x \in B \land x \notin C$.

    \item $X = \emptyset \iff (\forall x)[x \notin X]$.
  \end{enumerate}
\end{probsolution}
...
\begin{probsolution}
  \begin{enumerate}
    \item $X - A = \{x \in X \mid x \notin A\}$.
    \item $f^{-1}(B) = \{x \in A \mid f(x) \in B\}$.
    \item $A \cup B = \{x \mid x \in A \lor x \in B\}$.
    \item $f(A) = \{y \in B \mid (\exists x \in A)[f(x) = y]\}$.
    \item $C \cap f^{-1}(B) = \{x \in C \mid f(x) \in B\}$.
  \end{enumerate}
\end{probsolution}
