...
\begin{proof}
  Assume that $\sqrt{2}$ is rational, meaning that $(\exists p)(\exists q)[p \in
  \Z \land q \in \Z]$. Assume that $\sfrac{p}{q}$ is in simplest form, meaning
  $p$ and $q$ have no common factor. Then $\sqrt{2} = \sfrac{p}{q} \iff 2 =
  \sfrac{p^2}{q^2} \implies 2q^2 = p^2$. This means $p^2$ is an even integer.
  Then $(\exists r)[r \in \N]$ such that $p = 2r$, giving us $4r^2 = 2q^2$. This
...
\begin{theorem}
  Let $a, b \in \R$. Then $a = b$ if and only if $(\forall \epsilon)[\epsilon >
  0 \implies \lvert a - b \rvert < \epsilon]$.
\end{theorem}
...
true, then $S(n)$ is true, $(\forall n)[n \in \N]$.

\begin{question}
  Let $x_1 = 1$ and $x_{n+1} = \frac{1}{2} x_n + 1$. Show that $x_n$ is
  increasing, meaning, prove $(\forall n)[n \in \N \implies x_n \le x_{n+1}]$.
\end{question}
...
\begin{question}
  Show that $n^3 + 2n$ is divisible by $3$, $(\forall n)[n \in \N]$.
\end{question}
...
\begin{definition}[Upper Bound]
  A set $A \subseteq \R$ is bounded above if $(\exists b)(\forall a)[(b \in \R
  \land a \in A) \implies a \le b]$. The number $b$ is called the \textit{upper
  bound} for $A$.
\end{definition}
...
\begin{definition}[Maximum and Minimum]
  ...
  element of $A$ and $(\forall a)[a \in A \implies a_0 \ge a]$. Similarly, $a_0$
  is a \textit{minimum} of the set $A$ if $a_0$ is an element of $A$ and
  $(\forall a)[a \in A \implies a_0 \le a]$.
\end{definition}
...
\begin{nlemma}
  \label{lem:proving_upper_bound}

  Assume $s \in \R$ is an upper bound for a set $A \subseteq \R$. Then, $s =
  \sup(A)$ if and only if, $(\forall \epsilon)(\exists a)[(\epsilon > 0 \land a
  \in A) \implies s - \epsilon < a]$.
\end{nlemma}
...
\begin{proof} $ $
  \begin{enumerate}
    ...
    \item[$\impliedby$:] Assume $b$ is an upper bound for $A$. Assume $b < s
      \iff s - b > 0$. Let $\epsilon = s - b$. Then, $(\exists a)[a \in A
      \implies (s - \epsilon < a \iff b < a)]$, which is a contradiction, since
      $b$ is an upper bound. Therefore, $s = \sup(A)$. \qedhere
  \end{enumerate}
\end{proof}

\begin{question}
  Define $A = \left\{1 - \frac{1}{n} \mid n \in \N\right\}$. Using lemma
  \ref{lem:proving_upper_bound}, show that $\sup(A) = 1$.
\end{question}

\begin{proof}
  Assume $s = \sup(A) = 1$. Then $(\forall \epsilon)(\exists a)[(\epsilon > 0
  \land a = 1 - \sfrac{1}{n} \in A) \implies (s - \epsilon < a \iff 1 - \epsilon
  < 1 - \sfrac{1}{n} \iff 1 - (1 - \sfrac{1}{n}) < \epsilon \iff \sfrac{1}{n} <
  \epsilon)]$.
\end{proof}
