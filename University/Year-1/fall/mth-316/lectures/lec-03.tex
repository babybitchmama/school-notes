...
\begin{definition}[Upper Bound]
  ...
  b$, $\forall a \in A$. The number $b$ is called the \textit{upper bound} for
  $A$.
\end{definition}
...
(see figure \ref{fig:supremum_infimum_graphically} for a graphical
representation).
...
\begin{proof}
  By property $\circled{2}$ of the definition of the least upper bound, if $s$
  and $t$ are both least upper bounds for $A$, then $s \le t$ and $t \le s$,
  implying that $s = t$.
\end{proof}
...
\begin{definition}[Maximum and Minimum]
  ...
  \textit{minimum} of the set $A$ if $a_0$ is an element of $A$ and $a_0 \le a$,
  $\forall a \in A$.
\end{definition}
