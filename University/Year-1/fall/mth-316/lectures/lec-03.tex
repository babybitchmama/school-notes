...
\begin{proof}
  Assume that $\sqrt{2}$ is rational, meaning that $(\exists p \in \Z)(\exists q
  \in \Z)[2 = \sfrac{p}{q}]$. Assume that $\sfrac{p}{q}$ is in simplest form,
  meaning $p$ and $q$ have no common factor. Then $\sqrt{2} = \sfrac{p}{q} \iff
  2 = \sfrac{p^2}{q^2} \implies 2q^2 = p^2$. This means $p^2$ is an even
  integer. Then $(\exists r \in \N)[p = 2r]$, giving us $4r^2 = 2q^2$. This
  means $q^2$ is also an even integer, which is a contradiction, since $p$ and
  $q$ were assumed to be in simplest form. Therefore, $\sqrt{2}$ is irrational.
\end{proof}
...
\begin{proof} $ $
  \begin{enumerate}
    \item[$\implies$)] Let $\epsilon > 0$. Assume that $a = b$. Then $\lvert a -
      b \rvert = 0 < \epsilon$.
...
true, $(\forall n \in \N)[S(n)]$.

\begin{question}
  Let $x_1 = 1$ and $x_{n+1} = \frac{1}{2} x_n + 1$. Show that $x_n$ is
  increasing, meaning, prove $(\forall n \in \N)[x_n \le x_{n+1}]$.
\end{question}
...
\begin{proof}[Solution]
  ...
  Therefore, by the principle of mathematical induction, $(\forall n \in \N)[x_n
  \le x_{n+1}]$.
\end{proof}
...
\begin{proof}[Solution]
  ...
  Therefore, by the principle of mathematical induction, $(\forall n \in \N)[3
  \mid (n^3 + 2n)]$.
\end{proof}
...
This is referred to as the \textit{Axiom of Completeness}.
...
\begin{definition}[Upper Bound]
  A set $A \subseteq \R$ is bounded above if $(\exists b \in \R)(\forall a \in
  A)[a \le b]$. The number $b$ is called the \textit{upper bound} for $A$.
\end{definition}

\begin{definition}[Least Upper Bound]
  A real number $s$ is the \textit{least upper bound} for a set $A \subseteq \R$
...
The least upper bound is also called the \textit{supremum} of the set $A$. The
\textit{greatest lower bound}/\textit{infimum} for $A$ is also defined similarly
...
\begin{proof}
  By property (ii) of the definition of the least upper bound, if $s$ and $t$
...
\begin{example}
  Let $\displaystyle A = \left.\left\{\frac{1}{n}~\right\rvert n \in \N\right\}
  = \left\{1, \frac{1}{2}, \frac{1}{3}, \dots\right\}$. It's clear that $\sup(A)
  = 1$ and $\inf(A) = 0$.
\end{example}
...
\begin{definition}[Maximum and Minimum]
  A real number $a_0$ is a \textit{maximum} of the set $A$ if $a_0$ is an
  element of $A$ and $(\forall a \in A)[a_0 \ge a]$. Similarly, $a_0$ is a
  \textit{minimum} of the set $A$ if $a_0$ is an element of $A$ and $(\forall a
  \in A)[a_0 \le a]$.
\end{definition}

The closed interval $[a, b]$ has a maximum of $b$ and a minimum of $a$, since
they're both elements of the set. However, the open interval $(a, b)$ has
neither a maximum nor a minimum, since $a$ and $b$ aren't in the set.

\begin{example}
  Let $\displaystyle A = \left.\left\{1 + \frac{1}{n}~\right\rvert n \in
  \N\right\}$, giving us $\sup(A) = 2 = \max(A)$, but $\inf(A) = 1 \ne \min(A)$,
...
\begin{proof}[Solution]
  Let $s = \sup(A)$. Let $a \in A$. Since $s$ is an upper bound of $A$, then $a
  \le s \implies a + c \le s + c$ implies that $c + s$ is an upper bound of $c +
  A$. Then, $a \le s \implies a + c \le s + c \implies c + s$ is an upper bound
  of $c + A$. If $b$ is an upper bound of $A$, then $s \le b$. Now, let $d$ be
  an upper bound of $c + A$. Then, $c + a \le d \implies a \le d - c \iff d - c$
  is an upper bound of $A$. Then,
  \begin{align*}
...
\begin{lemma}
  ...
  \sup(A)$ if and only if, $(\forall \epsilon > 0)(\exists a \in A)[s - \epsilon
  < a]$.
\end{lemma}
...
\begin{proof} $ $
  \begin{enumerate}
    ...
    \item[$\impliedby$)] Assume $b$ is an upper bound for $A$. Assume $b < s
      \iff s - b > 0$. Let $\epsilon = s - b$. Then, $(\exists a \in A)[(s -
      \epsilon < a \iff b < a)]$, which is a contradiction, since $b$ is an
      upper bound. Therefore, $s = \sup(A)$. \qedhere
  \end{enumerate}
\end{proof}

\begin{question}
  Define $\displaystyle A = \left.\left\{1 - \frac{1}{n}~\right\rvert n \in
  \N\right\}$. Using the lemma, show that $\sup(A) = 1$.
\end{question}

\begin{proof}[Solution]
  Assume $s = \sup(A) = 1$. Then, $(\forall \epsilon > 0)(\exists a = 1 -
  \sfrac{1}{n} \in A)[s - \epsilon < a]$. Then, $1 - \epsilon < 1 -
  \sfrac{1}{n}$. This implies that $\epsilon > \sfrac{1}{n}$, which is true for
  all $n \in \N$. Therefore, $s = 1$.
\end{proof}
