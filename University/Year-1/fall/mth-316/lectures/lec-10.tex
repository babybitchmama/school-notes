...
\begin{example}[Alternating Series]
  Let $a_n = \frac{(-1)^n}{n}$, then $a_n \to 0$. But, the series
  $\sum_{n=1}^\infty \frac{(-1)^n}{n} = -\ln(2)$ by the Alternating Series Test
  (see section \ref{sub_sec:conversion_tests}).
\end{example}

\begin{example}[P-Series]
  Let $a_n = \frac{1}{n^\rho}$, then $a_n \to 0$. The series $\sum_{n=1}^\infty
  \frac{1}{n^\rho}$ converges if $\rho > 1$ and diverges if $\rho \le 1$.
\end{example}

% subsection types_of_series (end)
