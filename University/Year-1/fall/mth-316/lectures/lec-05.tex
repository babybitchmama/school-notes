...
\begin{definition}
  Let $f$ be a function $f : A \to B$. We say $f$ is \emph{onto} if $(\forall y \in B)(\exists x \in A)[f(x) = y]$. We say $f$ is \emph{one-to-one} if $(\forall x_1 \in A)(\forall x_2 \in A)[f(x_1) = f(x_2) \implies x_1 = x_2]$.
\end{definition}

\begin{example}\leavevmode
  \begin{enumerate}
    \item Let $A$ be the set of all the students in a room and $B$ be the set of all the chairs in the room. Then, $f$ is onto if every chair has a student, and $f$ is one-to-one if no two students are sitting in the same chair.

    \item The function $f : \N \to \{-1, 1\}$ defined by $f(n) = (-1)^n$ is onto, as all natural numbers are being mapped to either $-1$ or $1$. However, $f$ is not one-to-one, as $f(2) = f(4) = 1$. \qedhere
  \end{enumerate}
\end{example}

\begin{definition}
  Let $f$ be a function $f : A \to B$. If $f$ is both onto and one-to-one, then we call $f$ a \emph{one-to-one correspondence} of $A$ and $B$.
\end{definition}

\begin{note}
  Sometimes, onto is referred to as \emph{surjective}, one-to-one is referred to as \emph{injective}, and a one-to-one correspondence is referred to as a \emph{bijection}.
\end{note}
...
\begin{proof}
  This proof is relatively simple. Suppose that $\I$ is countable. Since we know that the union of two countable sets is countable and we know that $\Q$ is countable, then $\R = \Q \cup \I$ would imply that $\R$ is countable. This is a contradiction, as we've already shown that $\R$ is uncountable. Therefore, $\I$ is uncountable.
\end{proof}
