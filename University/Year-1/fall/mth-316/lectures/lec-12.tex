...
\begin{proof} $ $
  \begin{enumerate}
    \item[$\implies$)] Assume $(x_n) \to x$. Given $\epsilon > 0$, there exists
      an $N_1 \in \N$ such that $\lvert x_n - x \rvert < \frac{\epsilon}{2}$ for
...
\begin{enumerate}
  ...
  \item If $\liminf_{n \to \infty} a_n = \limsup_{n \to \infty} a_n = L$, then
    the sequence converges to $L$.

  \item $\limsup_{n \to \infty} a_n$ is finite if and only if the sequence is
    bounded above, and $\liminf_{n \to \infty} a_n$ is finite if and only if the
...
\begin{example}[Oscillating Sequence]
  ...
  \begin{align*}
    \sup_{k \geq n} a_k &= 1 \quad \text{(since $a_k = 1$ infinitely often)} \\
    \inf_{k \geq n} a_k &= -1 \quad \text{(since $a_k = -1$ infinitely often)}
  .\end{align*}
  Thus
  \begin{align*}
...
\subsection{Relationship to Cauchy Sequences}
\label{subsec:relationship_to_cauchy_sequences}
...
% subsection relationship_to_cauchy_sequences (end)

\subsection{Visual Intuition}
\label{subsec:visual_intuition}
...
% subsection visual_intuition (end)
