...
\begin{definition}[Limit Supremum and Infimum]
  Let $(a_n)$ be a sequence of real numbers. We define the \emph{limit supremum}
  and \emph{limit infimum} as
  \begin{align*}
...
\begin{remark}
  The \emph{limit supremum} (\emph{limsup}) is the largest value that the
  sequence approaches infinitely often, while the \emph{limit infimum}
  (\emph{liminf}) is the smallest value that the sequence approaches infinitely
  often.
\end{remark}
...
The \emph{limsup} and \emph{liminf} of a sequence provide insights into its
long-term behavior.
\begin{itemize}
...
The \emph{limsup} captures the ``upper boundary'' of eventual values of the
sequence, while the \emph{liminf} captures the ``lower boundary.''
