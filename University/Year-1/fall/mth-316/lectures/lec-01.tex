...
\begin{definition}[Set]
  ...
  A \textbf{set} is a collection of objects, called \textbf{elements} of the
  set.
\end{definition}
...
\begin{note}
  \label{nte:proper_definition_of_r}
...
\begin{note}
  \label{nte:and_or_notation}

  The notation $\land$ is the logical \textbf{and} operator, and $\lor$ is the
  logical \textbf{or} operator. Or in math is an inclusive or, meaning that the
...
\begin{note}
  \label{nte:if_and_only_if}

  The symbol $\iff$ means \textbf{if and only if}. The statement $A \iff B$ is
  shorthand notation for $(A \implies B) \land (B \implies A)$.
\end{note}

\begin{question}
  \label{qst:prove_set_equality}
...
\begin{solproof}
  \label{solprf:prove_set_equality}

  \begin{enumerate}
    \label{enum:prove_set_equality_solprf}
...
\begin{definition}[Complement]
  ...
  Given $A \subseteq \R$, the \textbf{complement} of $A$, written $A^c$, refers
...
\begin{theorem}[DeMorgan's Law]
  \label{thm:demorgans_law}

  Let $A$ and $B$ be sets. Then
  \begin{enumerate}
    \label{enum:demorgans_law_thm}
...
\begin{proof}
  \label{prf:demorgans_law} Suppose $A$ and $B$ are both sets.

  \begin{enumerate}
    \label{enum:demorgans_law_prf}
...
\begin{align}
  \begin{aligned}\label{eq:union_intersection_quantifiers}
    \empheq{
      x \in \bigcup_{n=1}^{\infty} A_n &\iff \exists n \in \N~\st~x \in A_n \\
      x \in \bigcap_{n=1}^{\infty} A_n &\iff \forall n \in \N~\st~x \in A_n
    }
  \end{aligned}
\end{align}

\Cref{eq:union_intersection_quantifiers} will be very useful to prove statements
