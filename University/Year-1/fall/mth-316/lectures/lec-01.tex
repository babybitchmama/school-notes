\lecture{1}{Sep 30 2024 Mon (13:01:47)}{Set theory}
...
\begin{note}
  The notation $\land$ is the logical \emph{and} operator, and $\lor$ is the
  logical \emph{or} operator. Or in math is an inclusive or, meaning that the
...
\begin{note}
  ...
  \end{enumerate}
  When I combine quantifiers, I do so as such $(\forall x \in S)(\exists y \in
  S)[P(x, y)]$. This means that for all $x$ in the set $S$, there exists a $y$
  in the set $S$ such that the statement $P(x, y)$ is true.
\end{note}
...
\subseteq A$.
\begin{note}
  The symbol $\iff$ means \emph{if and only if}. The statement $A \iff B$ is
...
\begin{definition}[Complement]
  Given $A \subseteq \R$, the \emph{complement} of $A$, written $A^c$, refers to
  the set of all elements of $\R$ not in $A$, written as
  \[%
