...
The definition begins with $\forall \epsilon > 0$. This means that we are trying
to prove that the sequence converges for all $\epsilon > 0$. Say, $\epsilon =
0.00000001$, or $\epsilon = 100000000$, and so on.
...
\begin{worksheet}
  ...
  that way, if not get closer. The $100$th term $a_{100} = \sfrac{1}{10}$ puts
  us right on the boundary, giving us
  \[%
  ...
  \begin{note}
    \emph{We aren't using a fixed $\epsilon$, since, say $\epsilon =
...
\begin{worksheet}
  ...
  we need to find a relationship between $\epsilon$ and $N$. Let's first try to
  solve for $n$ in the inequality $\lvert a_n - a \rvert < \epsilon$.
  \[%
...
\begin{worksheet}
  ...
  $\lvert a_n - a \rvert < \epsilon$ for all $n \ge N_1$. Same thing for $(a_n)
  ...
  \sfrac{\epsilon}{2}$. Same thing for $a_n \to b$. Using that, we get the new
  inequality
  \[%
...
\begin{proof}
  Let $\epsilon > 0$ be arbitrarily chosen. Since $a_n \to a$, there exists $N_1
  \in \N$ such that $\lvert a_n - a \rvert < \sfrac{\epsilon}{2}$ for all $n \ge
  N_1$. Similarly, since $a_n \to b$, there exists $N_2 \in \N$ such that
  $\lvert a_n - b \rvert < \sfrac{\epsilon}{2}$ for all $n \ge N_2$. Let $N =
