...
\begin{exersolution}[6]
  \begin{enumerate}
    ...
    \item \textbf{Proof:} Notice that $(a - c) + (c - d) + (d - b) = a - b$. We
      can use the triangle inequality for multiple terms $\lvert a - b \rvert =
      \lvert (a - c) + (c - d) + (d - b) \rvert \le \lvert a - c \rvert + \lvert
      c - d \rvert + \lvert d - b \rvert$. \hfill\qedsymbol

    \item \textbf{Proof:} Assume $\lvert a \rvert > \lvert b \rvert$ since
      $\lvert \lvert a \rvert - \lvert b \rvert \rvert$. Then $\lvert \lvert a
      \rvert - \lvert b \rvert \rvert = \lvert a \rvert - \lvert b \rvert =
      \lvert (a - b) + b \rvert - \lvert b \rvert \le \lvert a - b \rvert  +
      \lvert b \rvert - \lvert b \rvert = \lvert a - b \rvert$. \hfill\qedsymbol
  \end{enumerate}
\end{exersolution}
...
\begin{exersolution}[7]
  \begin{enumerate}
    \item The value of $f(A) = f([0, 2]) = [0, 4]$ and the value of $f(B) =
      ...
      We have $f(A \cup B) = f([0, 2] \cup [1, 4]) = f([0, 4]) = [0, 16]$ and
...
\begin{exersolution}[11]
  \begin{enumerate}
    ...
    \item There exists two real numbers $a < b$ such that if $c < b$ then $c <
      a$ for all $c \in \Q$. \textit{Intuition:} False.
  \end{enumerate}
\end{exersolution}
...
\begin{exersolution}[12]
  \begin{enumerate}
    \item \textbf{Proof:} I'll use mathematical induction.
      ...
      .\]%
      Therefore, by induction, $y_n > -6$ for all $n$. \hfill\qedsymbol
    ...
    \item \textbf{Proof:} I'll use mathematical induction.
      ...
      Induction step: Suppose that $y_n \ge y_{n+1}$ up to some number $k$.
      ...
      .\end{align*}
      Therefore, by induction, $y_n \ge y_{n+1}$, for all $n$. \hfill\qedsymbol
  \end{enumerate}
\end{exersolution}
...
\begin{exercise}[3]
  \begin{enumerate}
    ...
    \item Use \circled{1} to explain why there is no need to assert that
      greatest lower bounds exist as part of the Axiom of Completeness.
  \end{enumerate}
\end{exercise}

\begin{exersolution}[3]
  \begin{enumerate}
    \item By definition, $\sup(B)$ is the greatest lower bound for $A$, meaning
      that it equals $\inf(A)$.

    \item Part $\circled{1}$ proves that the greatest lower bound exists using
...
\begin{exersolution}[8]
  \begin{enumerate}
    \item $\sup(S) = 1$ and $\inf(S) = 0$.

    \item $\sup(S) = 1$ and $\inf(S) = -1$.

    \item $\sup(S) = \frac{1}{3}$ and $\inf(S) = \frac{1}{4}$.
