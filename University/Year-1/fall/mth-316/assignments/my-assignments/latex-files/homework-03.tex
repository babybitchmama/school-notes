...
\begin{exersolution}[3]
  \begin{enumerate}
    \item Find a college in the United States where all students are less than
      seven feet tall.

    \item Find a college in the United States where every professor does not
      only give A's or B's.

    \item Show that all colleges in the United States have at least one student
      that's shorter than six feet tall.
  \end{enumerate}
\end{exersolution}
...
\begin{exersolution}[4]
  \begin{enumerate}
    ...
    \item Impossible, if $\lim_{n \to \infty} a_n = a \ne 1$ then for any $n \ge
      N$ we can find a $n$ with $a_n = 1$, meaning $\lvert 1 - a \rvert <
      \epsilon$ is impossible.
...
\begin{exersolution}[6]
  \begin{proof}
    Let $\epsilon > 0$. Since $(a_n) \to a$, then, by the definition of the
    limit, we get $\exists N_1~\st~\forall n > N \implies \lvert a_n - a \rvert
...
\begin{exersolution}[1]
  \begin{enumerate}
    \item \begin{proof}
        Let $\epsilon > 0$. Since $(x_n) \to 0$, by the definition of
        convergence, there exists $N$ such that for all $n > N$, we get $\lvert
        x_n \rvert < \epsilon^2$. This means $x_n < \epsilon^2$ for a large
        enough $n$. Taking the square root of both sides we get $\sqrt{x_n} <
        \epsilon$ (this is allowed, as $x_n \ge 0 $).
    ...
    \item \begin{proof}
        Let $\epsilon > 0$. Since $(x_n) \to x$, by the definition of
        convergence, there exists $N$ such that for all $n > N$, we get $\lvert
        x_n - x \rvert < \epsilon^2$. Multiplying by $(\sqrt{x_n} + \sqrt{x})$
        gives $\lvert x_n - x \rvert < (\sqrt{x_n} + \sqrt{x})\epsilon$. Since
        $x_n$ is convergent, it is bounded $\lvert x_n \rvert \le M$ implying
        $\sqrt{\lvert x_n \rvert} \le \sqrt{M}$. This implies that
        \[%
...
\begin{exersolution}[3]
  \begin{proof}
    ...
    \sfrac{\epsilon}{2}$. Define $N = \max(\{N_1, N_2\})$. Then, for all $n >
    N$, we get $\lvert x_n - l \rvert < \sfrac{\epsilon}{2}$ and $\lvert z_n - l
    ...
    Thus, for all $n > N$, we get $\lvert y_n - l \rvert < \epsilon$, showing
    that if $\lim_{n \to \infty} x_n = z_n = l$ and $x_n \le y_n \le z_n$, then
