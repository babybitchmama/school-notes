...
\begin{exersolution}[12]
  \begin{enumerate}
    \item \textbf{Proof:} Suppose that $y_n > -6$. The base case is $y_1 = 6$,
      which is greater than $-6$. Assume it works until $n = k$. Then, let $n =
      k + 1$. We have
      \[%
        y_{n+1} = \frac{2y_n - 6}{3} > \frac{2(-6) - 6}{3} = -6
      .\]%
      Therefore, $y_{n+1} > -6$ for all $n \in \N$. \hfill\qedsymbol

    \item \textbf{Proof:} Suppose the base case is $y_{n+1} < y_n$, which is $2
      < 6$, which is true. Multiplying both sides of the inequality by $2$,
      subtracting by $6$, and dividing by $3$ gives us
      \[%
        \frac{2y_{n+1} - 6}{3} < \frac{2y_n - 6}{3}
      ,\]%
      showing us that $y_{n+2} < y_{n+1}$.

      Therefore, $(y_n)$ is decreasing. \hfill\qedsymbol
  \end{enumerate}
\end{exersolution}
...
\begin{exersolution}[13]
  \begin{enumerate}
    \item \textbf{Proof:} Exercise 1.2.5 will be our base case. Assume that the
      statement $(A_1 \cup A_2 \cup \cdots \cup A_n)^c = A_1^c \cap A_2^c \cap
      \cdots A_n^c$ is true. Then, we need to show that the statement is also
      true for $n + 1$, giving us
      \begin{emptyproof}
        \begin{align*}
