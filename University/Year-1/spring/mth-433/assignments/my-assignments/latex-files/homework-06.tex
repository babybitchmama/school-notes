...
\begin{solution}[(ii)]
  ...
  .\]%
  The graphs for each case are given in figure \ref{fig:sketch}.
\end{solution}
...
\begin{solution}[(iii)]
  ...
  Now, we deal with the domains of $v$ for each case.
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      % (ii) C < 1 and C = 1: domain is all real numbers
      \addplot [domain=-4:4, thick, teal] {1};
      \addplot [domain=-4:4, thick, teal] {2};

      % (ii) C > 1: domain is (-arcsin(1/C), arcsin(1/C)), say C = 2 => arcsin(0.5) ~ 0.523
      \addplot [domain=-0.523:0.523, thick, teal] {3};

      % (iii) Case 1: phi = C cosh v -> sinh(v) <= 1/C, use C = 2 => arcsinh(0.5) ~ 0.481
      \addplot [domain=-0.481:0.481, thick, purple] {4};

      % (iii) Case 2: phi = C sinh v -> cosh(v) <= 1/C, only if C <= 1
      % Use C = 0.8 => arccosh(1/0.8) ~ 0.693
      \addplot [domain=-0.693:0.693, thick, purple] {5};

      % (iii) Case 3: phi = e^v => 1 - e^{2v} >= 0 => v <= 0
      \addplot [domain=-4:0, thick, purple] {6};

      % Add dashed vertical lines for reference
      \draw[dashed] (axis cs:-0.523,0.5) -- (axis cs:-0.523,6.5);
      \draw[dashed] (axis cs:0.523,0.5) -- (axis cs:0.523,6.5);
      \draw[dashed] (axis cs:-0.481,0.5) -- (axis cs:-0.481,6.5);
      \draw[dashed] (axis cs:0.481,0.5) -- (axis cs:0.481,6.5);
      \draw[dashed] (axis cs:-0.693,0.5) -- (axis cs:-0.693,6.5);
      \draw[dashed] (axis cs:0.693,0.5) -- (axis cs:0.693,6.5);

      \node at (axis cs:-3,1.3) {\small Sphere/All $v$};
      \node at (axis cs:-3,3.3) {\small Bounded};
      \node at (axis cs:-3,6.3) {\small $v < 0$};

      \end{axis}
    \end{tikzpicture}

    \caption{Sketch of the profile of the surface in the $xz$-plane for the cases $C = 1$, $C > 1$, and $C < 1$.}
    \label{fig:sketch}
  \end{figure}

  For the first case, just like before, we have
  \[%
...
\newcommand\tih{\tilde{h}}
\begin{solution}[(i)]
  Let $\tih = h \circ \alpha$. Then,
  \[%
    H_ph(\w) = \odv[2]{\tih}{t}\bigg\vert_{t=0} = \odv{}{t}\bigg\vert_{t=0} \left[\pdv{\tih}{u} u'(t) + \pdv{\tih}{v} v'(t)\right] = \odv{}{t}\bigg\vert_{t=0} \left[\tih_u u'(t) + \tih_v v'(t)\right]
  .\]%
  By the product rule and chain rule, we compute
  \begin{align*}
    \odv{}{t}(\tih_u u') &= \tih_{uu} u' u' + \tih_u u'' = \tih_{uu} (u')^2 + \tih_u u'' \\
    \odv{}{t}(\tih_v v') &= \tih_{vv} (v')^2 + \tih_v v'' \\
    \odv{}{t}(\tih_u v') &= \tih_{uv} u' v' \quad \text{(and similarly } \odv{}{t}(\tih_v u') = \tih_{uv} v' u' \text{)}
  .\end{align*}
  Putting everything together,
  \begin{align*}
    H_ph(\w) &= \odv[2]{\tih}{t}\bigg\vert_{t=0} = \tih_{uu}(p)(u')^2 + 2\tih_{uv}(p)u'v' + \tih_{vv}(p)(v')^2 + \tih_u(p) u''(0) + \tih_v(p) v''(0)
  .\end{align*}
  Since \( p \) is a critical point of \( h \), we have \( \dd{h}_p = 0 \), which implies \( \tih_u(p) = h_u(p) = 0 \) and \( \tih_v(p) = h_v(p) = 0 \). Thus, the last two terms vanish, and we are left with
  \[%
    H_ph(\w) = \tih_{uu}(p)(u')^2 + 2\tih_{uv}(p)u'v' + \tih_{vv}(p)(v')^2
  .\]%
  This shows that \( H_p h : T_p(S) \to \R \) is a homogeneous degree-2 polynomial in the components of \( \w \), and hence a quadratic form. Since the expression depends only on second-order partials and the velocity vector \( \w = u'\x_u + v'\x_v \), it is independent of the chosen parametrization \( \x \), and therefore \( H_p h \) is a well-defined quadratic form on \( T_p(S) \).
\end{solution}

\begin{solution}[(ii)]
  Let $h(q) = \langle q - p, \Na(p) \rangle$ be the height function of $S$ relative to the tangent plane $T_p(S)$, where $\Na(p)$ is the unit normal vector to $S$ at $p$. Again, let $\tih(t) = (h \circ \alpha)(t)$. To verify that $p$ is a critical point of $h$, let $\alpha : (-\epsilon, \epsilon) \to S$ be any smooth curve on $S$ with $\alpha(0) = p$ and $\alpha'(0) = \w \in T_p(S)$. Then,
  \begin{align*}
    \tih(t) &= \langle \alpha(t) - p, \Na(p) \rangle \\
    \implies \odv{\tih}{t}\bigg\vert_{t=0} &= \left\langle \alpha'(0), \Na(p) \right\rangle = \left\langle \w, \Na(p) \right\rangle = 0
  ,\end{align*}
  since $\w \in T_p(S)$ and $\Na(p)$ is normal to $T_p(S)$. Thus, $\dd{h}_p(\w) = 0$ for all $\w \in T_p(S)$, so $p$ is a critical point of $h$, and hence $H_p h$ is well-defined.

  Now, differentiating again
  \[%
    H_p h(\w) = \odv[2]{\tih}{t}\bigg\vert_{t = 0} = \odv[2]{}{t} \langle \alpha(t) - p, \Na(p) \rangle \bigg\vert_{t=0} = \left\langle \alpha''(0), \Na(p) \right\rangle
  .\]%
  Since $\alpha$ lies on the surface $S$, and $\alpha''(0)$ is the acceleration vector of the curve at $p$, its normal component measures the curvature of $S$ in the direction of $\w$. Therefore,
  \[%
    H_p h(\w) = \langle \alpha''(0), \Na(p) \rangle = \text{normal curvature of}~S~\text{at}~p~\text{in the direction of} \w
  .\]%
  This is precisely the definition of the second fundamental form $\sff_p(\w, \w)$, and hence,
  \[%
    H_p h(\w) = \sff_p(\w, \w)
  .\]%
  Therefore, the Hessian at $p$ of the height function relative to $T_p(S)$ is equal to the second fundamental form at $p$.
\end{solution}
