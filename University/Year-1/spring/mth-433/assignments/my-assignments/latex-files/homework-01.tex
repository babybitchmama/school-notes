...
\begin{solution}[(ii)]
  Let $P = \alpha(t)$ be a point on the tractrix, and let $\v = \alpha'(t)$
  denote the tangent vector at $P$. Since $t$ is the angle that the $y$-axis
  makes with $\v$, we have
  \[%
    \v = \lVert \v \rVert \left\langle \sin(t), \cos(t) \right\rangle
  .\]%
  This means the direction of the tangent vector is $\left\langle \sin(t),
  \cos(t) \right\rangle$. Let $\ell$ denote the tangent line at $P$. The line
  $\ell$ passes through $P = \alpha(t)$ and is in the direction of $\v$, so its
  parametric form is
  \[%
    \ell(s) = \alpha(t) + s\left\langle \sin(t), \cos(t) \right\rangle
  .\]%

  To find where $\ell$ intersects the $y$-axis, we set the $x$-component of
  $\ell(s)$ to $0$
  \[%
    \sin(t) + s\sin(t) = 0 \implies s = -1
  .\]%
  Plugging back into $\ell(s)$, we find the $y$-coordinate of the point of
  intersection
  \begin{align*}
    y &= \cos(t) + \log\left(\tan\left(\frac{t}{2}\right)\right) - \cos(t) \\
      &= \log\left(\tan\left(\frac{t}{2}\right)\right)
  .\end{align*}
  So the point of intersection is
  \[%
    Q = \left\langle 0, \log\left(\tan\left(\frac{t}{2}\right)\right) \right\rangle
  .\]%

  Therefore, the segment of the tangent line between $P = \alpha(t)$ and the
  $y$-axis has length
  \begin{align*}
    \lVert \alpha(t) - Q \rVert &= \left\lVert \left\langle \sin(t), \cos(t) + \log\left(\tan\left(\frac{t}{2}\right)\right) - \log\left(\tan\left(\frac{t}{2}\right)\right) \right\rangle \right\rVert \\
                                &= \left\lVert \left\langle \sin(t), \cos(t) \right\rangle \right\rVert = \sqrt{\sin^2(t) + \cos^2(t)} = 1
  .\end{align*}
  Hence, the length of the segment of the tangent of the tractrix between the
  point of tangency and the $y$-axis is constantly equal to $1$.
\end{solution}

\begin{problem}[1.3.6]
  Let $\alpha(t) = \left\langle ae^{bt}\cos(t), ae^{bt}\sin(t) \right\rangle$,
  $t \in \R$, where $a > 0$ and $b < 0$, be a parametrized curve.
  \begin{enumerate}
...
\begin{solution}[(i)]
  Since $b < 0$, we have $e^{bt} \to 0$ as $t \to +\infty$. Therefore,
  \[%
    \alpha(t) = \left\langle ae^{bt}\cos(t), ae^{bt}\sin(t) \right\rangle \to (0,0) \qtq{as} t \to +\infty
  .\]%
  The presence of the $\cos(t)$ and $\sin(t)$ terms implies that $\alpha(t)$
  winds around the origin as it decays in magnitude. Thus, $\alpha$ spirals into
...
\begin{solution}[(ii)]
  ...
  .\end{align*}
  Since $e^{bt} \to 0$ as $t \to +\infty$, it follows that
  \[%
  ...
  \begin{align*}
    \int_{t_0}^t \lVert \alpha'(x) \rVert \dx &= \int_{t_0}^t \sqrt{(abe^{bx} \cos x - ae^{bx} \sin x)^2 + (abe^{bx} \sin x + ae^{bx} \cos x)^2} \dx \\
                                              &= \int_{t_0}^t ae^{bx} \sqrt{(b \cos x - \sin x)^2 + (b \sin x + \cos x)^2} \dx
  .\end{align*}
  Now simplify the expression under the square root
  \begin{align*}
    (b \cos x - \sin x)^2 + (b \sin x + \cos x)^2 &= b^2 \cos^2 x - 2b \cos x \sin x + \sin^2 x + b^2 \sin^2 x + 2b \sin x \cos x + \cos^2 x \\
                                                  &= b^2(\cos^2 x + \sin^2 x) + (\sin^2 x + \cos^2 x) \\
                                                  &= b^2 + 1
  .\end{align*}
  So we have
  \[%
    \int_{t_0}^t \lVert \alpha'(x) \rVert \dx = \int_{t_0}^t ae^{bx} \sqrt{b^2 + 1} \dx = a\sqrt{b^2 + 1} \int_{t_0}^t e^{bx} \dx
  .\]%
  Evaluate the integral
  \[%
    a\sqrt{b^2 + 1} \int_{t_0}^t e^{bx} \dx = a\sqrt{b^2 + 1} \cdot \frac{1}{b} \left( e^{bt} - e^{bt_0} \right)
  .\]%
  Taking the limit as $t \to +\infty$
  \[%
    \lim_{t \to +\infty} \int_{t_0}^t \lVert \alpha'(x) \rVert \dx = a\sqrt{b^2 + 1} \cdot \frac{1}{b} \left( 0 - e^{bt_0} \right) = -\frac{a\sqrt{b^2 + 1}}{b} e^{bt_0} < \infty
  .\]%
  Therefore, the arc length of $\alpha$ over $[t_0, \infty)$ is finite.
\end{solution}
...
\begin{solution}[(i)]
  ...
  that for any vectors $\a$ and $\b$, $\a \cdot \b \le \lvert \a \rvert \lvert
  \b \rvert$. Applying this to $\alpha'(t)$ and $\v$, we have $\alpha'(t) \cdot
  \v \le \lVert \alpha'(t) \rVert$. Therefore, we have
  \[%
...
\begin{problem}[1.4.2]
  ...
  0$. Show that the vector $\v = \langle a, b, c \rangle$ is perpendicular to
  the plane and that $\sfrac{\lvert d \rvert}{\sqrt{a^2 + b^2 + c^2}}$ measures
...
\begin{proof}[Solution]
  ...
  We now compute the distance from the origin $(0, 0, 0)$ to the plane. Let $\v
  = \langle a, b, c \rangle$ be the normal vector to the plane and let $\p$ be
  any point on the plane $P$, so that $a x + b y + c z + d = 0$. Then the vector
  $\p$ points from the origin to a point on the plane, and the distance from the
  origin to the plane is given by projecting $\p$ onto the normal vector
  \[%
    D = \lvert \proj_\v(\p) \rvert = \frac{\lvert \p \cdot \v \rvert}{\lVert \v \rVert}
  .\]%
  Since $\p \cdot \v = -d$, we have
  \[%
    D = \frac{\lvert -d \rvert}{\lVert \v \rVert} = \frac{\lvert d \rvert}{\sqrt{a^2 + b^2 + c^2}}
  ,\]%
...
\begin{solution}
  We are given two linearly independent vectors $\u, \v \in \R^2$ written in
  terms of the standard basis $\{ \e_1, \e_2 \}$ as
  \[%
    \u = u_1\e_1 + u_2\e_2 \aand \v = v_1\e_1 + v_2\e_2
  .\]%
  Then the matrix with $u$ and $v$ as rows is
  \[%
    \begin{pmatrix}
      u_1 & u_2 \\
      v_1 & v_2
    \end{pmatrix}
  ,\]%
  and the matrix with $u$ and $v$ as columns is
  \[%
    \begin{pmatrix}
      u_1 & v_1 \\
      u_2 & v_2
  ...
  Now consider the product of these two matrices
  \[%
    \begin{pmatrix}
      u_1 & u_2 \\
      v_1 & v_2
    ...
    \end{pmatrix}
  .\]%
  This matrix appears on the left-hand side of the problem statement and encodes
  the inner products of $\u$ and $\v$.

  The area $A$ of the parallelogram spanned by $\u$ and $\v$ satisfies
  \[%
    A^2 = \lVert \u \rVert^2 \lVert \v \rVert^2 - (\u \cdot \v)^2
  ,\]%
  which is the determinant of the Gram matrix above
  \[%
    A^2 = \begin{vmatrix}
      \u \cdot \u & \u \cdot \v \\
      \v \cdot \u & \v \cdot \v
    \end{vmatrix}
  .\]%
  Substituting the matrix identity from earlier, we obtain
  \[%
    A^2 =
    \begin{vmatrix}
      u_1 & u_2 \\
  ...
  Since the square of the determinant is always non-negative, we can use the
  sign of the determinant itself to determine the orientation of the basis
  $\{\u, \v\}$. Therefore, the signed or \textit{oriented} area is given by
  \[%
    A = \begin{vmatrix}
      u_1 & u_2 \\
      v_1 & v_2
    \end{vmatrix}
  ,\]%
  where $A > 0$ if $\{\u, \v\}$ is positively oriented, and $A < 0$ if it is
  negatively oriented. This completes the proof.
\end{solution}
...
\begin{solution}[(iv)]
  ...
  .\end{align*}
  Solving either equation (assuming $\cos(\sfrac{s}{c}) \neq 0$ or
  ...
  \begin{align*}
    \v &= \left(0,0, b\frac{s}{c}\right) - \alpha(s)
...
\begin{solution}[(iii)]
  We know that $\Ba(s) = \Ta(s) \wedge \Na(s)$ and that $\odv{\Ba}/{s} =
  -\tau(s) \Na(s)$, from the Frenet–Serret formulas. Our goal is to express the
  torsion $\tau(t)$ of the original parametrization $\alpha(t)$ in terms of its
  derivatives with respect to $t$.

  Recall that $\beta(s) = \alpha(t(s))$, and so
  \[%
    \Ta(s) = \odv{\beta}{s} = \frac{\alpha'(t)}{\lVert \alpha'(t) \rVert},\quad \odv{\Ta}{s} = \kappa(s)\Na(s),\aand \odv{\Ba}{s} = -\tau(s)\Na(s)
  .\]%

  We now compute $\odv{\Ba}/{s}$ directly in terms of $\alpha$. First, recall
  that $\Ba = \Ta \wedge \Na$, and use the product rule for derivatives
  \begin{align*}
    \odv{\Ba}{s} &= \odv{}{s}(\Ta \wedge \Na) \\
                &= \odv{\Ta}{s} \wedge \Na + \Ta \wedge \odv{\Na}{s}
  .\end{align*}

  Using the Frenet–Serret formulas again,
  \[%
    \odv{\Ta}{s} = \kappa \Na \aand \odv{\Na}{s} = -\kappa \Ta + \tau \Ba
  ,\]%
  so
  \begin{align*}
    \odv{\Ba}{s} &= \kappa \Na \wedge \Na + \Ta \wedge (-\kappa \Ta + \tau \Ba) \\
                &= 0 + \tau \Ta \wedge \Ba
  .\end{align*}

  Since $\Ba = \Ta \wedge \Na$, and the Frenet frame $\{\Ta, \Na, \Ba\}$ is a
  positively oriented orthonormal basis, we know that $\Ta \wedge \Ba = -\Na$.
  Hence,
  \[%
    \odv{\Ba}{s} = \tau (-\Na) = -\tau \Na
  .\]%

  Now, to write $\tau$ in terms of $\alpha$, recall that
  \[%
    \Ba = \frac{\alpha'(t) \wedge \alpha''(t)}{\lVert \alpha'(t) \wedge \alpha''(t) \rVert}
  ,\]%
  and
  \begin{align*}
    \odv{\Ba}{s} &= \odv{}{s} \left( \frac{\alpha' \wedge \alpha''}{\lVert \alpha' \wedge \alpha'' \rVert} \right) \\
                &= \odv{t}{s} \cdot \odv{}{t} \left( \frac{\alpha' \wedge \alpha''}{\lVert \alpha' \wedge \alpha'' \rVert} \right)
  .\end{align*}

  Differentiating the numerator
  \[%
    \odv{}{t}(\alpha' \wedge \alpha'') = \alpha' \wedge \alpha''' + \alpha'' \wedge \alpha''
    = \alpha' \wedge \alpha'''
  ,\]%
  since $\alpha'' \wedge \alpha'' = 0$. Then
  \begin{align*}
    \odv{\Ba}{s} &= \frac{1}{\lVert \alpha' \rVert} \cdot \odv{}{t} \left( \frac{\alpha' \wedge \alpha''}{\lVert \alpha' \wedge \alpha'' \rVert} \right) \\
                 &= \frac{1}{\lVert \alpha' \rVert} \cdot \left( \frac{\alpha' \wedge \alpha'''}{\lVert \alpha' \wedge \alpha'' \rVert}
                   - \frac{(\alpha' \wedge \alpha'') \cdot (\alpha' \wedge \alpha''')}{\lVert \alpha' \wedge \alpha'' \rVert^3}
                 (\alpha' \wedge \alpha'') \right)
  .\end{align*}

  Taking the dot product with $\Na = \Ba \wedge \Ta$, we isolate the component
  in the $\Na$ direction. Since
  \[%
    \odv{\Ba}{s} = -\tau \Na
  ,\]%
  and $\Na$ is perpendicular to $\Ba$ and $\Ta$, we can dot both sides with
  $\Na$ to get
  \begin{align*}
    \tau &= -\odv{\Ba}{s} \cdot \Na \\
        &= -\frac{1}{\lVert \alpha' \rVert} \cdot \left( \frac{\alpha' \wedge \alpha'''}{\lVert \alpha' \wedge \alpha'' \rVert} \cdot \Na \right)
  .\end{align*}

  Since $\Ba = \frac{\alpha' \wedge \alpha''}{\lVert \alpha' \wedge \alpha''
  \rVert}$ and $\Na = \Ba \wedge \Ta$, this leads us to
  \[%
    \tau(t) = -\frac{(\alpha' \wedge \alpha'') \cdot \alpha'''}{\lVert \alpha' \wedge \alpha'' \rVert^2}
  .\]%
...
\begin{solution}
  ...
  .\]%
  Using $\alpha(t_0) = \lVert \alpha(t_0) \rVert \Na(t_0)$, we compute
  \begin{align*}
