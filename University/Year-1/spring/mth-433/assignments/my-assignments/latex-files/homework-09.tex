...
\begin{proof}
  ...
  For the first case, all the normals are the same, which means that the surface is contained in a plane. For the second case,
  the Gauss map $N : S \to S^2$ has image contained in a great circle on the unit sphere.
  This implies that the surface is a portion of a sphere.
...
\begin{solution}
  ...
  \[%
    \widetilde{X} := \dd{\varphi} \circ X, \quad \widetilde{Y} := \dd{\varphi} \circ Y
  .\]%
  ...
  \[%
    X\bra{Y, Z} = \bra{\nabla_X Y, Z} + \bra{Y, \nabla_X Z}
  .\]%
  Define a vector field $\widetilde{Z} := \dd{\varphi} \circ Z$ for some arbitrary vector field $Z$ on $S_1$. Then
  \begin{align*}
