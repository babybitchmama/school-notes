...
\begin{solution}
  ...
  cover it. Consider the open set $U_1 = \{(x, y, z) \in \R^3 | 0 < y < \pi\}$
  and $U_2 = \{(x, y, z) \in \R^3 \mid 0 < x^2 + y^2 < 1\}$. Define $\Phi_1 :
  U_1 \to \R^2$ and $\Phi_2 : U_2 \to \R^2$ by
  \[%
    \Phi_1(x, y, z) = (x, z) \aand \Phi_2(x, y, z) = (\theta, z)
  .\]%
\end{solution}
...
% \begin{solution}[(i)]
%   To derive the formula for $\pi^{-1} : \mathbb{R}^2 \to S^2$, consider the line
