...
\begin{exercise}[7.8]\label{ex:7.8}
  Show that the axioms of a 2-category imply the following equalities.
  \begin{equation}\label{eq:7.8:interchange:1}
    ...
    \end{tikzpicture}
  \end{equation}
  \begin{equation}\label{eq:7.8:interchange:2}
    \begin{tikzpicture}[baseline]
      \draw (-1,1) -- (1,1);
...
\begin{solution}[7.8]\label{sol:7.8}
  The diagram \ref{eq:7.8:interchange:1} follows from the axioms of a 2-category. In particular, the interchange law states that horizontal and vertical compositions of 2-morphisms are compatible. More precisely, given 2-morphisms $\alpha: F_1 \implies F_2$ and $\beta: G_1 \implies G_2$, the horizontal composition $\beta \ast \alpha : G_1 \circ F_1 \implies G_2 \circ F_2$ is defined, and satisfies:
  \[%
    (\beta' \circ \beta) \ast (\alpha' \circ \alpha) = (\beta' \ast \alpha') \circ (\beta \ast \alpha)
  .\]%
  In \ref{eq:7.8:interchange:1}, each equality reflects this interchange identity. That is, we may either first compose vertically and then horizontally, or compose horizontally first and then vertically, and the result is the same. This coherence condition is one of the core structural axioms of any strict 2-category.

  For diagram \ref{eq:7.8:interchange:2}, this identity expresses the fact that the horizontal composition of 2-morphisms with identity 1-morphisms on either side is strictly associative, and that such composition is natural. Specifically, suppose:
  \[%
...
\begin{exercise}[7.16]\label{ex:7.16}
...
\begin{solution}[7.16(i)]\label{sol:7.16i}
...
\begin{solution}[7.16(ii)]\label{sol:7.16ii}
...
\begin{exercise}[7.17]\label{ex:7.17}
...
\begin{solution}[7.17(i)]\label{sol:7.17i}
  ...
  ,\end{align*}
  loop evaluation, symmetry of cups and caps, and sliding. I've already shown the snake identities in \ref{sol:7.16i} and the loop evaluation in \ref{sol:7.16ii}.
\end{solution}

\begin{solution}[7.17(ii)]\label{sol:7.17ii}
\end{solution}

\begin{solution}[7.17(iii)]\label{sol:7.17iii}
\end{solution}

\begin{exercise}[7.19]\label{ex:7.19}
...
\begin{solution}[7.19]\label{sol:7.19}
\end{solution}

\begin{exercise}[7.20]\label{ex:7.20}
...
\begin{solution}[7.20]\label{sol:7.20}
\end{solution}
