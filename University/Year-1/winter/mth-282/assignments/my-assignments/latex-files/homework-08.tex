...
\begin{proof}[Solution]
  ...
  positive ($+1$), the normal vector is already pointing upward. To orient it
  downward, we take the negative of the normal vector, $\NN = \langle -4, -1, -1
  \rangle$. Therefore, the surface element is given by
  \[%
    \dS = \langle -4, -1, -1 \rangle \,\dA
  .\]%
  Computing the dot product, we get
  \[%
    \FF \cdot \dS = (x - 2y)(-4) + (2z + 8x)(-1) + (y + z)(-1) = 10y - 36
  .\]%
...
\begin{proof}[Solution]
  In spherical coordinates, we have $x = 3\sin(\theta)\cos(\phi)$, $y =
  3\sin(\theta)\sin(\phi)$, and $z = 3\cos(\theta)$. Since we're considering the
  positive half of $y$, we have $0 \le \theta \le \pi$ and $-\sfrac{2}{\pi} \le
  \phi \le \sfrac{2}{\pi}$ for the upper hemisphere. Parametrizing, we get
  \[%
    \r = \langle 3\sin(\theta)\cos(\phi), 3\sin(\theta)\sin(\phi), 3\cos(\theta) \rangle
  .\]%
  We know that
  \begin{align*}
    \r_\theta &= \langle 3\cos(\theta)\cos(\phi), 3\cos(\theta)\sin(\phi), -3\sin(\theta) \rangle \\
    \r_\phi &= \langle -3\sin(\theta)\sin(\phi), 3\sin(\theta)\cos(\phi), 0 \rangle
  .\end{align*}
  Therefore, the normal vector is given by
  \begin{align*}
    \NN = \r_\theta \times \r_\phi &= \begin{vmatrix}
      \ui & \uj & \uk \\
      3\cos(\theta)\cos(\phi) & 3\cos(\theta)\sin(\phi) & -3\sin(\theta) \\
      -3\sin(\theta)\sin(\phi) & 3\sin(\theta)\cos(\phi) & 0
    \end{vmatrix} \\
                                   &= \langle 9\sin^2(\theta)\sin(\phi), 9\sin^2(\theta)\sin(\phi), 9\cos(\theta)\cos(\phi)\sin(\theta)\cos(\phi) + 9\cos(\theta)\sin(\phi)\sin(\theta)\sin(\phi) \rangle \\
                                   &= \langle 9\sin^2(\theta)\sin(\phi), 9\sin^2(\theta)\sin(\phi), 9\cos(\theta)\sin(\theta)\cos^2(\phi) + 9\cos(\theta)\sin(\theta)\sin^2(\phi) \rangle \\
                                   &= \langle 9\sin^2(\theta)\cos(\phi), 9\sin^2(\theta)\sin(\phi), 9\cos(\theta)\sin(\theta) \rangle
  .\end{align*}
  Since the surface is oriented in the positive $y$-axis, we take the negative
  of the normal vector, $\NN = \langle -9\sin^2(\theta)\cos(\phi),
  -9\sin^2(\theta)\sin(\phi), -9\cos(\theta)\sin(\theta) \rangle$. Therefore,
  the surface element is given by
  \begin{align*}
    \dS &= \lvert \NN \rvert \,\dd{\theta} \,\dd{\phi} \\
        &= 9\sqrt{\sin^4(\theta)\cos^2(\phi) + \sin^4(\theta)\sin^2(\phi) + \cos^2(\theta)\sin^2(\theta)} \,\dd{\theta} \,\dd{\phi} \\
        &= 9\sin(\theta)\sqrt{\sin^2(\theta)\cos^2(\phi) + \sin^2(\theta)\sin^2(\phi) + \cos^2(\theta)} \,\dd{\theta} \,\dd{\phi} \\
        &= 9\sin(\theta)\sqrt{\sin^2(\theta)(\cos^2(\phi) + \sin^2(\phi)) + \cos^2(\theta)} \,\dd{\theta} \,\dd{\phi} \\
        &= 9\sin(\theta)\sqrt{\sin^2(\theta) + \cos^2(\theta)} \,\dd{\theta} \,\dd{\phi} \\
        &= 9\sin(\theta) \,\dd{\theta} \,\dd{\phi}
  .\end{align*}
  Computing the dot product, we get
  \[%
    \FF \cdot \NN = 81\cos(\theta)\sin^3(\theta)\cos^2(\phi) + 27\sin^3(\theta)\cos(\phi)\sin(\phi) + 27\cos(\theta)\sin^2(\theta)\sin(\phi)
  .\]%
  Therefore, we get
  \begin{align*}
    \iint_S \FF \cdot \dd{\S} &= \int_0^\pi \int_{-\sfrac{\pi}{2}}^{\sfrac{\pi}{2}} 81\cos(\theta)\sin^3(\theta)\cos^2(\phi) + 27\sin^3(\theta)\cos(\phi)\sin(\phi) + 27\cos(\theta)\sin^2(\theta)\sin(\phi) \,\dd{\phi} \,\dd{\theta} \\
                              &= \textrm{BEAUTIFUL MATH STUFF} \\
                              &= 0
  .\qedhere\end{align*}
\end{proof}
