...
%   \begin{enumerate}
%     \item $\FF = \langle xy^2 + 4xy, 2y + x^2 \rangle$ and $C$ is the path $y =
...
%   \[%
%     \pdv{Q}{x} - \pdv{P}{y} = \left(2x\right) - \left(2xy + 4x\right) = 2xy - 2x
%   .\]%
...
%   \begin{align*}
%     \oint_C \FF \cdot \dd{\r} &= \iint_D \left(\pdv{Q}{x} - \pdv{P}{y}\right) \,\dA \\
%                               &= \int_{-2}^1 \int_{x^2}^{-x+2} 2xy - 2x \,\dy \,\dx \\
%                               &= \int_{-2}^1 xy^2 - 2xy\bigg\vert_{x^2}^{-x+2} \,\dx \\
%                               &= \int_{-2}^1 \left[x(-x + 2)^2 - 2x(-x + 2)\right] - \left[x(x^2)^2 - 2x(x^2)\right] \,\dx \\
%                               &= \int_{-2}^1 \left[x^3 - 2x^2\right] - \left[x^5 - 2x^3\right] \,\dx \\
%                               &= \int_{-2}^1 -x^5 + 3x^3 - 2x^2 \,\dx \\
%                               &= -\frac{x^6}{6} + \frac{3x^4}{4} - \frac{2x^3}{3}\bigg\vert_{-2}^1 \\
%                               &= \left[-\frac{(1)^6}{6} + \frac{3(1)^4}{4} - \frac{2(1)^3}{3}\right] - \left[-\frac{(-2)^6}{6} + \frac{3(-2)^4}{4} - \frac{2(-2)^3}{3}\right] \\
%                               &= -\frac{27}{4}
%   .\qedhere\end{align*}
% \end{proof}
...
%   \[%
%     \pdv{Q}{x} - \pdv{P}{y} = \left(\sin(x)\right) - \left(\sin(x) - 4y^3\right) = -4y^3 = -4r^3\sin^3(\theta)
%   .\]%
...
%   \begin{align*}
%     \oint_C \FF \cdot \dd{\r} &= \iint_D \left(\pdv{Q}{x} - \pdv{P}{y}\right) \,\dA \\
%                               &= \int_0^\pi \int_0^2 -4r^3\sin^3(\theta) \cdot r \,\dr \,\dd{\theta} \\
...
%                &= \langle r, -2r\cos(\theta), 2r^2\sin(\theta) \rangle
%   \end{align*}
%   The magnitude of the normal vector is
%   \begin{align*}
%     \lvert \r_r \times \r_\theta \rvert &= \sqrt{r^2 + 4r^2\cos^2(\theta) + 4r^2\sin^2(\theta)} \\
%                                          &= \sqrt{r^2 + 4r^2} = \sqrt{5r^2} = r\sqrt{5}
%   .\end{align*}
%   Therefore, the surface area element is
%   \[%
%     \dS = \lvert \r_r \times \r_\theta \rvert \,\dA = r\sqrt{5} \,\dr \,\dd{\theta}
%   .\]%
%   The surface area integral is
%   \begin{align*}
%     A &= \int_0^{2\pi} \int_0^4 r\sqrt{5} \,\dr \,\dd{\theta} \\
%       &= \sqrt{5} \int_0^{2\pi} \,\dd{\theta} \cdot \int_0^4 r \,\dr \\
%       &= 2\pi\sqrt{5} \cdot \frac{4^2}{2} = 16\pi\sqrt{5}
%   .\qedhere\end{align*}
% \end{proof}
...
\begin{proof}[Solution to (iii)]
  Using cylindrical coordinates, we have
  \[%
    x = r\cos(\theta), \quad y = y, \aand z = r\sin(\theta)
  ,\]%
  we get the parametric equation for the surface area as
  \[%
    \r = \langle 2\cos(\theta), y, 2\sin(\theta) \rangle
  .\]%
  The bounds for $y$ is $-2 \le y \le 2$ and $0 \le \theta \le 2\pi$.

  We know that
  \begin{align*}
    \r_r &= \left\langle \pdv{x}{r}, \pdv{y}{r}, \pdv{z}{r} \right\rangle = \langle \cos(\theta), 0, \sin(\theta) \rangle \\
    \r_\theta &= \left\langle \pdv{x}{\theta}, \pdv{y}{\theta}, \pdv{z}{\theta} \right\rangle = \langle -r\sin(\theta), 0, r\cos(\theta) \rangle
  .\end{align*}
  The normal vector is
  \begin{align*}
    \mathbf{N} &= \r_r \times \r_\theta = \begin{vmatrix}
    ...
    \end{vmatrix} \\
               &= \langle (0)(r\cos(\theta)) - (\sin(\theta))(0), -\left[(\cos(\theta))(r\cos(\theta)) - (\sin(\theta))(0)\right], (r\cos(\theta))(0) - (0)(-r\sin(\theta)) \rangle \\
               &= \langle 0, -r\cos^2(\theta), 0 \rangle
  \end{align*}
  The magnitude of the normal vector is
\end{proof}
