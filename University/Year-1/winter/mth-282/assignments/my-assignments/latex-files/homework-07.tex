...
\begin{proof}[Solution to (iii)]
  ...
  .\]%
  The bounds for $y$ are $0 \le y \le 2$ and $0 \le \theta \le 2\pi$.
  ...
  The normal vector is
  \begin{align*}
    \mathbf{N} &= \r_y \times \r_\theta = \begin{vmatrix}
      \mathbf{i} & \mathbf{j} & \mathbf{k} \\
      0 & 1 & 0 \\
      -2\sin(\theta) & 0 & 2\cos(\theta)
    \end{vmatrix} \\
               &= \langle (1)(2\cos(\theta)), -\left[(0)(2\cos(\theta)) - (0)(-2\sin(\theta))\right], (0)(0) - (1)(-2\sin(\theta)) \rangle \\
               &= \langle 2\cos(\theta), 0, 2\sin(\theta) \rangle
  .\end{align*}
  The magnitude of the normal vector is
  \begin{align*}
    \lvert \r_y \times \r_\theta \rvert &= \sqrt{(2\cos(\theta))^2 + (2\sin(\theta))^2} \\
                                         &= \sqrt{4\cos^2(\theta) + 4\sin^2(\theta)} = \sqrt{4} = 2
  .\end{align*}

  Therefore, the surface area element is
  \[%
    \dS = \lvert \r_y \times \r_\theta \rvert \,\dA = 2 \,\dy \,\dd{\theta}
  .\]%

  The surface area integral is
  \begin{align*}
    A &= \int_0^{2\pi} \int_0^2 2 \,\dy \,\dd{\theta} \\
      &= 2 \int_0^{2\pi} \,\dd{\theta} \cdot \int_0^2 \,\dy \\
      &= 4\pi \cdot 2 = 8\pi
  .\qedhere\end{align*}
\end{proof}
