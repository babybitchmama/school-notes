...
\begin{proof}[Solution to (i)]
  ...
  $\alpha_2 = \lambda\alpha_3$, and so on. If $\alpha_1 \ne 0$, then $\lambda =
  0$ making the entire eigenvector zero. If $\alpha_1 = 0$, then $0 =
  \lambda\alpha_2$. Applying this recursively, we get $\alpha_1 = \alpha_2 =
  \alpha_3 = \cdots = 0$. Therefore, there are no eigenvalues or eigenvectors
  for $T$.
\end{proof}

\begin{proof}[Solution to (ii)]
  Suppose $\lambda$ and $\v$ are an eigenvalue and eigenvector of $U$,
  ...
  .\]%
  This implies that $\alpha_2 = \lambda\alpha_1$, $\alpha_3 = \lambda\alpha_2$,
  $\alpha_4 = \lambda\alpha_3$, and so on. If $\alpha_1 = 0$, then $\alpha_2 =
  \alpha_3 = \alpha_4 = \cdots = 0$. If $\alpha_1 \ne 0$, then $\lambda =
  \sfrac{\alpha_2}{\alpha_1}$. Applying this recursively, we get
  \[%
    \lambda = \frac{\alpha_2}{\alpha_1} = \frac{\alpha_3}{\alpha_2} = \frac{\alpha_4}{\alpha_3} = \cdots
  .\]%
  Therefore, $0 \ne \alpha_1 = \alpha_2 = \alpha_3 = \alpha_4 = \cdots$. Thus,
  the eigenvalues of $U$ are all non-zero real numbers and the eigenvectors are
  \[%
    \v = (\alpha, \alpha, \alpha, \cdots) = \alpha(1, 1, 1, \cdots), \quad \forall\alpha \in \R - \{0\}
  .\qedhere\]%
\end{proof}
...
\begin{problem}[10]
  Let $T$ be a linear transformation on an $n$-dimensional vector space $V$, and
  suppose that $T$ has $n$ distinct eigenvalues. Prove that $T$ is
  diagonalizable.
\end{problem}

\begin{proof}[Solution]
  A linear transformation is diagonalizable if there exists a basis of $V$
  consisting of eigenvectors of $T$. This means that $V$ has a basis of $n$
  linearly independent eigenvectors. Since $T$ has $n$ distinct eigenvalues,
  $\lambda_1, \cdots, \lambda_n$, there exist corresponding eigenvectors
  $\v_1, \cdots, \v_n$ such that
  \[%
    (\forall i = 1, \cdots, n)[T(\v_i) = \lambda_i \v_i]
  .\]%
  We will show that these eigenvectors are linearly independent, as shown in my
