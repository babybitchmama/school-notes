...
\begin{proof}[Solution]
  Let $V$ be a vector space. Let $G \subseteq V$ such that $\Sspan(G) = V$ and
  $\lvert G \rvert = n$. If $L \subseteq V$ is an empty set or linearly
  independent set with $\lvert L \rvert = n$, then
  \begin{enumerate}
    \item $m \le n$.
    \item There exists $H \subseteq G$ with $\lvert H \rvert = n - m$ such that
      $\Sspan(H \cup L) = V$. \qedhere
  \end{enumerate}
\end{proof}
...
\begin{proof}[Solution]
  ...
  Let $\e$ and $\f$ be zero vectors in $V$. Since $(\forall \a \in V)[\e + \a =
  \a]$, then, we have $\e + \f = \f$. Since $(\forall \b \in V)[\b + \f = \b]$,
  then, we have $\e + \f = \e$. Therefore, $\e = \f$.
\end{proof}
...
\begin{proof}[Solution to (iii)]
  Yes, by definition, $W$ is a subspace of if it's not empty and closed under
  addition and scalar multiplication. But since $W$ is already a vector space,
  that means it's not empty and closed under addition and scalar multiplication,
