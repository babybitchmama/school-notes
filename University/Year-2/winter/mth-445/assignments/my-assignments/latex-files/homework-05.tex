...
\begin{solution}
  In order for $\Z_3[x]/\braket{f_c(x)}$ to be a field, where $f_c(x) = x^3 + x^2 + c$, we must have $f_c(x)$ be an irreducible polynomial. We check each of the values $c = 0, 1, 2$.
  For $c = 0$, we have $f_0(x) = x^3 + x^2$, which is reducible, since it has a root at $x = 0$.
  For $c = 1$, we have $f_0(x) = x^3 + x^2 + 1$, which is reducible, since it it has a root at $x = 1$.
  For $c = 2$, we have $f_2(x) = x^3 + x^2 + 2$, which is irreducible, since it doesn't have any roots.
  Therefore, for $\Z_3[x]/\braket{x^3 + x^2 + c}$ to be a field, we must have $c = 2$.
\end{solution}
...
\begin{solution}
  We can construct the idea $I = p\Z \times \Z$, where $p = 2$ is a prime number. We now check that it is an ideal of $\Z \times \Z$. Clearly, $I$ is commutative, since the original ring, $\Z \times \Z$ is commutative.
  It is also non-empty, since we have $(2, n) \in I$. Now we check that it's
  closed.
\end{solution}
