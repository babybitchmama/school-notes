...
\begin{document}
  ...
  \begin{frame}{What is a Metric?}
    ...
    \begin{itemize}
      ...
      \item The metric determines how derivatives behave.
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}{The Graph of $u$ vs. The Surface Itself}
    ...
    \begin{itemize}
      ...
      \item This is a 2D surface sitting \emph{extrinsically} inside 3D space.
    \end{itemize}

    \vspace{0.3cm}
    \textbf{The Surface defined by the metric $g = u(x,y)(dx^2 + dy^2)$:}
    \begin{itemize}
      \item This defines the \emph{intrinsic} geometry of a 2D space.
  ...
  \begin{frame}{Curvature of the Fubini-Study Metric (Cont.)}
    ...
    \begin{align*}
      K &= -\frac{1}{2u}\Delta (\ln u) \\
        &= -\frac{1}{2 \left( \frac{4}{(1+r^2)^2} \right)} \left( -\frac{8}{(1+r^2)^2} \right) \\
        &= \frac{(1+r^2)^2}{8} \cdot \frac{8}{(1+r^2)^2} \\
        &= 1
    .\end{align*}
