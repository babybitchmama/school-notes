...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item (Harder) Calculate the pushforward of the vector fields $\pd{z}$ and $\pd{\bar{z}}$ under the inverse map $\phi^{-1}$. Use this to derive the formula for the Laplacian in polar coordinates
      ...
      \textbf{Solution:} Computing the inverse map, we have $\phi^{-1}(x, y) = (r, \theta)$, where $r = \sqrt{x^2 + y^2}$ and $\theta = \arctan(y/x)$. Letting $z = x + iy$ and $\bar{z} = x - iy$. Computing $\pd{z}$ and $\pd{\bar{z}}$ in terms of $\pd{x}$ and $\pd{y}$, we have:
      \[%
    ...
    % \item Compute the line integral of a geodesic on the cone with the metric $g_\beta$ around the singularity.

    %   \textbf{Solution:} We first parametrize the geodesic $\gamma$ on a cone with the given metric $g_\beta$. A natural choice for such a geodesic is a circle of radius $r$ centered at the cone's apex. We can parametrize this circle as
    %   \[%
    %     \gamma(x, y) = (r(x,y), y(x, y), \theta(x, y)) = (r, \theta) = (r, t)~\text{for}~t \in [0, 2\pi)
    %   .\]%
  \end{enumerate}
\end{document}
