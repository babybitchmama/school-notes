...
\subsection{The Tangent and Cotangent Bundles} Our next main definition is that of a tensor, but in order to do so, we need to introduce a few more concepts, such as the tangent and cotangent bundles of a manifold.
\begin{definition}
...
\begin{definition}
  The \emph{cotangent space} $T_p^*M$ is the dual space to $T_pM$. If $\{ \frac{\partial}{\partial x^i} \}$ is the coordinate basis for the tangent space, we define the \emph{coordinate dual basis} $\{ \dx^i \}$ for $T_p^*M$ such that:
  \[%
    \dx^i \left( \pdv{}{x^j} \right) = \delta^i_j
  ,\]%
  where $\delta^i_j$ is the Kronecker delta. This allows us to express any covector $\omega \in T_p^*M$ as $\omega = \omega_i \dx^i$.
\end{definition}

\begin{note}
  If $\{\pdv{}/{x^i}\}$ is a basis for $T_pM$, then the dual basis $\{\dx^i\}$ is a basis for $T_p^*M$.
\end{note}
...
\subsection{Tensors and Sections}
...
\subsection{Conformal and Isometric Maps} We end this section by defining conformal and isometric maps, which are important concepts in the study of geometric inverse problems.
