...
We start this section by developing the necessary mathematical tools and concepts required to understand geometric inverse problems. This section just briefly introduces concepts from differential that's used throughout this paper.
...
\todo[inline]{Fix this recursive definition, since we're using the definition of fibre of a vector bundle to define the tangent bundle, which is a vector bundle.}
...
\includefigure{./chapters/chapter-01/figures/fibres-and-sections.tex}
...
\begin{definition}
  ...
  \[%
    T_p : (T_p^*M)^r \times (T_pM)^s \to \mathbb{R}.
  .\]%
\end{definition}
...
\[%
  T = T^{i_1 \dots i_r}_{j_1 \dots j_s} \left(\bigotimes_{k=1}^r \pd{i_k}\right) \otimes \left(\bigotimes_{m=1}^s \dx^{j_m}\right)
.\]%
...
\begin{example}\leavevmode
  \begin{itemize}
    \item \textbf{(0,0)-Tensors (Scalar Fields):} An $(0,0)$-tensor is simply a smooth function $f : M \to \R$. In medical imaging, this may represent an isotropic physical property, such as the \emph{proton density} in Magnetic Resonance Imaging (MRI) or the \emph{absorption coefficient} in optical tomography.

    \item \textbf{(1,0)-Tensors (Vector Fields):} A $(1,0)$-tensor is a smooth section of the tangent bundle $TM$. Locally, it is written as $X = X^i \pd{i}$. These represent directional quantities, such as the \emph{velocity field} of blood flow in Doppler ultrasound or the \emph{gradient} of a potential (once identified via a metric).

    \item \textbf{(0,1)-Tensors (Covectors/1-forms):} A $(0,1)$-tensor is a smooth section of the cotangent bundle $T^*M$, written locally as $\omega = \omega_i \dx^i$. A primary example is the \emph{differential of a function} $\df$, which at each point acts as a linear functional on tangent vectors.

    \item \textbf{(0,2)-Tensors (Bilinear Forms):} A $(0,2)$-tensor is a section of $T^*M \otimes T^*M$. This is perhaps the most critical category for this paper, as it includes the \emph{Riemannian metric} $g$. In \emph{anisotropic} media, like heart muscle or brain white matter, physical properties like electrical conductivity or water diffusion are not single numbers but are modeled as symmetric $(0,2)$-tensors $g_{ij}$.
  \end{itemize}
\end{example}
...
\[%
  (v')^\alpha = \pdv{F^\alpha}{x^i} v^i
.\]%
Thus, the matrix of $\dF_p$ with respect to the coordinate bases is the Jacobian matrix $\pdv{F^\alpha}/{x^i}$.
...
\subsection{Conformal and Isometric Maps} We conclude this section by defining conformal and isometric maps, which are the fundamental ``equivalence relations'' used to categorize Riemannian manifolds in geometric inverse problems.
...
The distinction between these two maps is the key to understanding the ``up to a diffeomorphism'' clause in the Boundary Rigidity Problem.

\begin{itemize}
  \item \textbf{Isometries as Gauge Freedom:} If $F$ is an isometry that is the identity on the boundary, then the boundary distance functions $d_g$ and $d_h$ are identical. This means that a scanner cannot distinguish between $(M, g)$ and its isometric twin $(M, F^*h)$.
  \item \textbf{Conformal Factors as Diagnostic Data:} In Ultrasound CT, the goal is often to find the conformal factor $\Omega = 1/c(x)$. Since a change in $\Omega$ changes the travel times (lengths) of geodesics, this information is \emph{not} hidden by gauge freedom and can be uniquely reconstructed under the ``simple manifold'' assumptions.
\end{itemize}
