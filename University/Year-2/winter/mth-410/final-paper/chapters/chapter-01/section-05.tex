...
% talk about the The Koszul Formula
...
% Show how \Gamma is actually used to differentiate a vector field V = V^k \pd{k} (this is the actual formula people use in imaging simulations): (\nabla_XV)^k = X^i (\pd{i}V^k + \Gamma^k_{ij} V^j)
Now, let's use the Christoffel symbols to compute the covariant derivative of a vector field $V = V^k \pd{k}$ in the direction of another vector field $X = X^i \pd{i}$.
Let $V = V^k\pd{k}$ be a smooth vector field and $X = X^i\pd{i}$ be the direction of differentiation. By the Leibniz rule (property of the connection), the covariant derivative $\nabla_X V$ is:
\[%
