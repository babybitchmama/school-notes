...
Because $\alpha_g$ preserves the canonical symplectic form on the cotangent bundle, it implies that the phase-space volume of a beam of geodesics is conserved as it passes through the manifold. Geometrically, this means that while the metric $g$ may focus or disperse rays (lensing), it cannot ``create'' or ``destroy'' information unless the manifold fails to be simple. For instance, in the \textbf{Spherical Cap} example from Section \ref{sub_sec:examples}, the lens data would show a converging effect, where a wide array of incoming vectors is mapped to a narrow set of exit points, eventually collapsing into a singularity at a conjugate point.
