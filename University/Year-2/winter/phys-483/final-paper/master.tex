...
\begin{document}
  ...
  If you already have a strong understanding of differential geometry (\ref{sub_sec:differential_geometry}), partial differential equations (\ref{sub_sec:partial_differential_equations}), and linear algebra (\ref{sub_sec:linear_algebra}), you can skip to section~\ref{sec:theoretical_physics_background}, where we introduce the necessary physics background for understanding gravitational waves.
  ...
  We start this section by developing the necessary mathematical tools and concepts required to understand gravitational waves. This includes a brief review of linear algebra, differential geometry, and partial differential equations.

  % 1. Linear Algebra (for understanding the properties of tensors, etc.)
  \subsection{Linear Algebra}
  \label{sub_sec:linear_algebra}

  % 2. Differential Geometry (manifolds, tensors, curvature, curvature flow, etc.)
  \subsection{Differential Geometry}
  \label{sub_sec:differential_geometry}
  ...
  Let $M$ be a manifold
  \footnote{
    Throughout this paper, we're only interested in smooth manifolds, so we will assume that all manifolds are smooth unless otherwise stated. We also drop the adjective ``$n$-dimension''. So, it's safe to assume that whenever we say ``manifold'', we mean a smooth manifold of dimension $n$, unless otherwise stated.
  }.
  We can define something called a metric that allows us to talk about distances and angles on the manifold. This is crucial for understanding the geometry of spacetime and the behavior of gravitational waves. We can define a metric as follows.
  \begin{definition}
  ...
  In local coordinates, tensor components are written using upper and lower indices to distinguish components with respect to a chosen basis of $T_pM$ and its dual basis in $T_p^*M$. We go over this in more detail in section~\ref{sec:theoretical_physics_background}.
  ...
  % 3. Partial Differential Equations (wave equations, hyperbolic PDEs, etc.)
  \subsection{Partial Differential Equations}
  \label{sub_sec:partial_differential_equations}
