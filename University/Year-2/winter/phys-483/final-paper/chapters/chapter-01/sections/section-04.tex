...
\label{sub_sec:levi_civita_connection}
...
\label{sub_sec:parallel_transport_and_geodesics}
...
.\]%
Applying the Euler-Lagrange equations to $E[\gamma]$ yields the exact same coordinate geodesic equation.

\todo[inline]{Change this to reference gravtiational waves instead of medical imaging techniques.}
In the context of our boundary rigidity problem, this variational perspective is vital; it tells us that ultrasound rays (geodesics) always follow the paths that minimize ``acoustic effort'' (time-of-flight), which is the very data we measure at the boundary.

\subsection{Curvature}
\label{sub_sec:curvature}
Next, we introduce the concept of curvature on a manifold.
...
\subsection{Curvature Contractions} The Riemann tensor possesses 256 components in four dimensions, but its inherent symmetries reduce this to only 20 independent components. These symmetries are essential for performing contractions correctly.
