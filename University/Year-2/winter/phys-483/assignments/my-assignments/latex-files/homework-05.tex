...
\begin{solution}[(i)]
  ...
  \[%
    l^0 = \frac{1 + 2\Phi}{1 - 2\Phi} u^iv^i
  .\]%
\end{solution}

\begin{solution}[(ii)]
  Using the fact that $\l$ is parallel transported, we get:
  \[%
    \odv{l^i}{t} + \Gamma_{\alpha\beta}^\mu \dot{x}^\alpha l^\beta = 0
  .\]%
  Expanding using the definition of the Christoffel symbols in the metric $g_{\mu\nu}$, we get:
  \[%
    \odv{l^i}{t} + \frac{1}{2}g^{\mu\sigma}\left(\pd{\alpha}g_{\sigma\beta} + \pd{\beta}g_{\sigma\alpha} - \pd{\sigma}g_{\alpha\beta}\right) \dot{x}^\alpha l^\beta = 0
  .\]%
\end{solution}
...
\begin{solution}[(i)]
  For a 2D sphere of radius $r_0$, the non-vanishing components of the Riemann curvature tensor are:
  \begin{gather*}
...
\begin{solution}[(ii)]
  ...
  \begin{align*}
    \int_{\S^2} \sqrt{g} \dd{\phi,\theta} R &= \int_0^{2\pi} \int_0^\pi r_0^2\sin(\theta) \dd{\phi,\theta} \frac{2}{r_0^2} \\
                                               &= 2 \int_0^{2\pi} 2 \dd{\phi} \\
                                               &= \frac{4r_0^2[2\pi - 0]}{r_0^2} = 8\pi
  .\qedhere\end{align*}
\end{solution}
...
\begin{problem}[3 (Birkhoff's Theorem)]
  ...
  \begin{enumerate}
    ...
    \item Using the expressions for the Einstein tensor ($'$ denotes derivative with respect to to $r$ while $\dot{\phantom{ }}$ denotes derivative with respect to t)
      \begin{align*}
        G_{\hat{t}\hat{t}} &= e^{-\lambda}\left(-1 + e^\lambda + r\lambda'\right)/2 \\
