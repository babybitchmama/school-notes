...
\begin{solution}[(i)]
  ...
  \begin{alignat*}{3}
    \hat{x} &= r\cos(\theta) \quad &&(\text{pointing East}) \\
    \hat{y} &= r\sin(\theta) \quad &&(\text{pointing North}) \\
    \hat{z} &= r\cos(\phi) &&(\text{pointing Upward})
  .\end{alignat*}
  For the torsion balance to be in mechanical equilibrium, the net torque acting on the system must be zero. This means that the sum of all torques due to gravitational forces, centrifugal forces, and any other external influences must cancel out.
\end{solution}
