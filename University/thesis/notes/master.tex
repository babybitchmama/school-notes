...
\begin{document}
  ...
  \section{Geometric and Analytic Preliminaries}

  \subsection{Definitions}

  \begin{definition}[Lebegue Space]
  ...
  \begin{definition}[Sobolev Space]
    If $L^q(M)$ is the Lebesgue space, for some nonnegative integer $k$, the \emph{Sobolev Space}, $L^q_k(M)$ is the set of $u \in L^q(M)$ such that $Pu = f \in L^q(M)$ whenever $P$ is a smooth differential operator of order $\le k$. The \emph{Sobolev norm} on $L^q_k(M)$ is given by
  ...
  \begin{definition}[Space of $C^k$ Functions]
    \emph{The space of $C^k$ functions}, $C^k(M)$, is the set of functions on $M$ that are $k$-times continuously differentiable. The \emph{norm} on $C^k(M)$ is given by
  ...
  \begin{definition}[H\"older Space]
    ...
    \begin{equation}\label{eq:holder_norm}
      \|u\|_{C^{k,\alpha}} \coloneqq \|u\|_{C^k} + \sup_{x,y} \frac{\left\lvert \nabla^k u(x) - \nabla^k u(y)\right\rvert}{\left\lvert x - y \right\rvert^\alpha}
    ,\end{equation}
    is finite.
  \end{definition}
  ...
  \begin{computational-work}[Theorem~\ref{thm:aubin}]\leavevmode
    ...
    \begin{enumerate}
      \item Showing $\|\phi\|_p^2 = \|\phi^2\|_{p/2}$:
        \[%
          \|\phi\|_p^2 = \left(\int_M |\phi|^p \dd{V}_g\right)^{2/p} = \left(\int_M |\phi|^p \dd{V}_g\right)^{1/(p/2)} = \|\phi^2\|_{p/2}
        .\]%
      ...
      \item Showing $\left\lVert \sum \psi_i^2 \phi^2\right\rVert \le \sum_i \left(\int_M |\psi_i \phi|^p \dd{V}_g\right)^{2/p}$:
        ...
        .\end{align*}
      \item Showing $\sum_i \left(\int_M |\psi_i \phi|^p \dd{V}_g\right)^{2/p} \le (1 + \epsilon)^{2/p} \sum_i \left(\int_{U_i} |\psi_i \phi|^p \dd{V}_g\right)^{2/p}$:
        \[%
          \sum_i \left(\int_M |\psi_i \phi|^p \dd{V}_g\right)^{2/p} = \sum_i \left(\int_{U_i} |\psi_i \phi|^p \dd{V}_g + \int_{M - U_i} |\psi_i \phi|^p \dd{V}_g\right)^{2/p}
        .\]%
        The eigenvalues of $g_{jk}$ are between $(1 + \epsilon)^{-1}$ and $(1 + \epsilon)$, so the volume form $\dd{V}_g$ is between $(1 + \epsilon)^{-n/2} \dx$ and $(1 + \epsilon)^{n/2} \dx$. Since $p = 2n/(n - 2)$, we have $n/2 = p/(p - 2)$, so
        \[%
          \dd{V}_g \le (1 + \epsilon)^{n/2} \dx = (1 + \epsilon)^{p/(p - 2)} \dx = (1 + \epsilon)^{2/p} \dx
        .\]%
        Therefore,
        \[%
          \int_{M - U_i} |\psi_i \phi|^p \dd{V}_g \le (1 + \epsilon)^{2/p} \int_{M - U_i} |\psi_i \phi|^p \dx
        .\]%
        Thus, we have:
        \begin{align*}
          \sum_i \left(\int_M |\psi_i \phi|^p \dd{V}_g\right)^{2/p} &\le (1 + \epsilon)^{2/p} \sum_i \left(\int_{U_i} |\psi_i \phi|^p \dx + \int_{M - U_i} |\psi_i \phi|^p \dx\right)^{2/p} \\
                                                                    &\le (1 + \epsilon)^{2/p} \sum_i \left(\int_{U_i} |\psi_i \phi|^p \dx\right)^{2/p}
        .\end{align*}
      \item Simplifying $|\nabla(\psi_i \phi)|^2$:
        \begin{align*}
          |\nabla(\psi_i \phi)|^2 &= \braket{\nabla(\psi_i \phi), \nabla(\psi_i \phi)} \\
                                  &= \braket{\psi_i \nabla \phi + \phi \nabla \psi_i, \psi_i \nabla \phi + \phi \nabla \psi_i} \\
                                  &= |\psi_i|^2 |\nabla \phi|^2 + 2\psi_i \phi \braket{\nabla \psi_i, \nabla \phi} + |\phi|^2 |\nabla \psi_i|^2
        .\end{align*}
        Working with the middle term, we have:
        \[%
          2\psi_i \phi \braket{\nabla \psi_i, \nabla \phi} \le 2\psi_i|\nabla \phi| \cdot \phi|\nabla \psi_i|
        .\]%
        Letting $a = \psi_i|\nabla \phi|$ and $b = \psi|\nabla \psi_i|$, we have:
  ...
  \begin{note}
    For more details of the computational work used in the proof of Theorem~\ref{thm:aubin}, see Computational Work~\ref{cw:aubin}.
  \end{note}
  ...
  \begin{note}
    For more details of the computational work used in the proof of Theorem~\ref{thm:local_elliptic_regularity}, see Computational Work~\ref{cw:local_elliptic_regularity}.

    I'll also be proving the theorem for the $L^2$ case, as I don't have the machinery to handle the $L^q$ case.
  \end{note}
  ...
  \begin{theorem}[Global Elliptic Regularity]
    ...
    \begin{enumerate}
      \item If $f \in L^q_k(M)$, then $u \in L^q_{k+2}(M)$, and
        \[%
          \|u\|_{q,k+2} \le C\left(\|u\|_{q,k} + \|u\|_q\right)
        .\]%
      \item If $f \in C^{k,\alpha}(M)$, then $u \in C^{k+2,\alpha}(M)$, and
        \[%
          \|u\|_{C^{k+2,\alpha}} \le C\left(\|u\|_{C^{k,\alpha} + \|u\|_{C^\alpha}}\right)
        .\]%
    \end{enumerate}
  \end{theorem}
  ...
  \section{The Model Case: The Sphere}
